...
class Beelzebub {
     ...
     * @returns {Promise}
     */
    // TODO: re arch this to run the task from with in the class
    // so it can recursavily chain down to resolve promises all the way down
    _runPromiseTask(parent, task) {
        ...
        if( _.isString(task) ) {
            ...
            task = this._rootTasks[taskName];
            p = this._rootTasks[taskName].$runTask(taskParts.join('.'));
        }
...
class BaseTasks {
    ...
    // TODO: move to recursive run model
    $runTask(task) {
        let p = null;
        ...
        return p.then( () => {
            ...
            if( _.isString(task) ) {
                ...
                if( this._subTasks[taskName] ) {
                    return this._subTasks[taskName].$runTask(taskParts.join('.'));
                }
